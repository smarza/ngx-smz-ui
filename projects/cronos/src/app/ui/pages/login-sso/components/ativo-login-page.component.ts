...
import { getFirst, GlobalInjector, TenantsSelectors } from '@ngx-smz/core';
...
@Component({
    ...
    <div class="bg-gradient-to-b from-[#0B3F54] to-[#021722] dark:from-surface-50 dark:to-surface-100 flex items-center justify-center min-h-screen min-w-[100vw] overflow-hidden">
        ...
        <div class="flex flex-row items-center justify-center gap-20">
          <img [src]="layoutService.isDarkTheme() ? 'cronos-login-dark.png' : 'cronos-login-light.svg'" alt="logo" class="mx-auto max-w-sm">
          <div class="flex flex-col items-start justify-start gap-4">
            ...
            <p-select id="tenant" appendTo="body" [options]="tenants()" [(ngModel)]="tenant" (onChange)="onTenantChange($event)" optionLabel="name" optionValue="alias" placeholder="Selecione uma unidade de negócio" class="w-full" />
        ...
        <div class="flex flex-col md:flex-row items-center justify-center gap-20">
          ...
          <div class="flex flex-col items-start justify-start gap-4 w-full md:min-w-96">
            <div class="text-surface-0 dark:text-surface-900 text-lg font-medium mb-4">Login</div>
            <label for="tenant" class="block text-surface-0 dark:text-surface-900 text-md italic">Unidade de Negócio</label>
            <p-select id="tenant" appendTo="body" [options]="tenants()" [(ngModel)]="tenant" (onChange)="onTenantChange($event)" optionLabel="name" optionValue="alias" placeholder="Selecione uma unidade de negócio" class="w-full" styleClass="w-full" />
