...
@Component({
  selector: 'app-rti-scenario-settings',
  standalone: true,
...
export class RtiScenarioSettingsComponent implements OnInit {
  private readonly store: Store = inject(Store);
  @Input() public scenario: ScenarioDetails;
  public general: UpdateScenarioWithRtiStrategy;
  public teams: FixedTeamData[];
  public formData: StrategyFormData<UpdateScenarioWithRtiStrategy, FixedTeamData>;
  ...
  public ngOnInit(): void {
    this.defaultEditing();
    this.hasRtiHidroblastCoverage = this.scenario.results?.hasRtiHidroblastCoverage;
  }

  public getGeneralData(general: UpdateScenarioWithRtiStrategy): void {
  ...
  public enableEditing(): void {
    ...
    setTimeout(() => {
      ...
      this.formData = getRtiStrategyFormData(this.scenario, annualPlanning, this.isEditing);
    }, 0);
  }
  ...
  public updateScenario(): void {
    const payload: UpdateScenarioWithRtiStrategy = {
      ...this.general,
  ...
  public confirmPlanRecalculation(payload: UpdateScenarioWithRtiStrategy): void {
    this.store.dispatch(new ScenariosFtActions.UpdateScenarioWithRtiStrategy(payload))
  ...
  public confirmScenarioRecalculation(payload: UpdateScenarioWithRtiStrategy): void {
    this.store.dispatch(new ScenariosFtActions.UpdateScenarioWithRtiStrategy(payload))
      .subscribe(() => this.location.back());
  }
}