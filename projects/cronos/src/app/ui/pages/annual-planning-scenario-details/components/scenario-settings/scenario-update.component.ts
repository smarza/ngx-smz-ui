      ...
      <p-tabPanel>
        <ng-template pTemplate="header">
          <div class="grid grid-nogutter items-center justify-start gap-2">
            ...
            <p-button icon="fa-solid fa-trash" (onClick)="removeTeam(i)" [rounded]="true" [text]="true" [disabled]="!allowEditing || teamTabs.length === 1"/>
          </div>
        </ng-template>
...
export class ScenarioUpdateComponent implements OnInit, OnDestroy {
  ...
  @Output() public isValid: EventEmitter<unknown> = new EventEmitter();
  public showHidroblastLocationsTree = true;
  ...
  public ngOnInit(): void {
    this.data.teamForms.forEach((teamForm: {teamData: SmzForm<unknown>, hidroblastLocations: string[]}) => {
      this.teamTabs.push({
        form: { ...teamForm.teamData },
        tree: this.data.hidroblastLocationsTree,
        primeSelectionState: cloneDeep(teamForm.hidroblastLocations),
        selected: cloneDeep(teamForm.hidroblastLocations),
        isValid: false
      });
    });
    this.checkProximity();
  ...
  public addTeam(): void {
    this.teamTabs.push({
      form: { ...this.data.defaultTeamForm },
      tree: this.data.hidroblastLocationsTree,
  ...
  private checkValidity(): void {
    ...
    const isTeamHidroblastValid = this.teamTabs.every(team => team.selected.length > 0) || this.showHidroblastLocationsTree === false;
    const isValid = isGeneralFormValid && isTeamFormValid && isTeamHidroblastValid;
