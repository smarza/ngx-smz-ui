...
@Component({
  selector: '[smz-ui-diamond-notifications]',
  changeDetection: ChangeDetectionStrategy.OnPush,
  template: `
    <a (click)="toggle()">
      <i class="pi pi-bell"></i><span class="topbar-badge">{{ items.length }}</span>
    </a>
    <ul *ngIf="isExpanded" class="notifications-menu fade-in-up" smz-ui-diamond-notification-items (onClose)="collapse()" [items]="items"></ul>
  `,
})
export class DiamondNotificationsComponent implements OnInit {
