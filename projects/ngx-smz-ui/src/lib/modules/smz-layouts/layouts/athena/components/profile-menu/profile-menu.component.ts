import { AfterViewInit, ChangeDetectionStrategy, Component, ContentChildren, ElementRef, HostBinding, inject, Input, OnInit, QueryList } from "@angular/core";
...
@Component({
    ...
    template: `
      ...
      <a class="profile clickable grid items-center justify-end m-0 p-0 flex-nowrap gap-2" [ngClass]="{ 'profile-with-icon': !uiConfig.layouts.useAvatar }" (click)="toggle()">
        @if (userData$ | async; as userdata) {
          ...
          @if (uiConfig.layouts.useAvatar && userdata[uiConfig.layouts.avatarProperty]) {
            <img [src]="(userdata[uiConfig.layouts.avatarProperty] ) | safeUrl" class="profile-image">
          }
      ...
      @if (isExpanded) {
        <ul class="topbar-menu fadeInDown topbar-menu-visible" smz-ui-athena-profile-menu-items [items]="profile" (collapse)="isExpanded = false"></ul>
      }
      `,
    standalone: false
})
export class AthenaProfileMenuComponent implements OnInit, AfterViewInit {
  ...
  public userData$ = inject(Store).select(AuthenticationSelectors.userdata);
