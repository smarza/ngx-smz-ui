...
@Component({
  ...
  template: `
      ...
      <a class="profile clickable grid items-center justify-end m-0 p-0 flex-nowrap gap-2" [ngClass]="{ 'profile-with-icon': !config.useAvatar }" (click)="toggle()">
        <ng-container *ngIf="userData$ | async as userdata">
          ...
          <i *ngIf="!config.useAvatar || (config.useAvatar && !userdata[config.avatarProperty])" class="fa-solid fa-circle-user profile-icon profile-icon-menu"></i>
