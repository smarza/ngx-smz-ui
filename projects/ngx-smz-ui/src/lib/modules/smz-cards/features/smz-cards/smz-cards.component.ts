import { AfterContentInit, AfterViewInit, ChangeDetectionStrategy, ChangeDetectorRef, Component, ContentChildren, ElementRef, inject, Input, OnInit, QueryList, Renderer2, TemplateRef, ViewChild, ViewEncapsulation } from '@angular/core';
    ...
    if (!isEmpty(this.state.view.dataViewContentStyles))
    {
      ...
      if (this.dataView != null)
      {
        ...
        styles
          .filter(x => !isEmpty(x))
          .forEach(style => {
            this.renderer.addClass(container.firstElementChild, style);
           });
      }
