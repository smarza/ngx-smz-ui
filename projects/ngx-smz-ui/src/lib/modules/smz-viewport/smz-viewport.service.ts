...
export class SmzViewportService {
  ...
  public register(target: Element, config: SmzViewportConfig): void {
  ...
  public unregister(target: Element, config: SmzViewportConfig): void {
    this.ngZone.runOutsideAngular(() => {
      const foundedEntry = this.findEntry(config.root, config.hash);
      if (foundedEntry) {
